(function(app) {
	app.PriceShortRouteReportComponent =
		ng.core.Component({
		selector: 'price-short-route-report',
		templateUrl: 'views/price-short-route-report.html',
		})
		.Class({
	...
	app.PriceShortRouteReportComponent.prototype.ngOnInit=function(){
	...
	app.PriceShortRouteReportComponent.prototype.checkRol=function(){
		this.actions=[];
		this.vistas=[];
		var flag=false;
			...
			if(this.service.getRole().hasOwnProperty("views")){
				if(!(this.service.getRole().views==null || this.service.getRole().views==undefined || this.service.getRole().views=="")){
					...
					for(var i=0;i<tabla.length;i++){
						if(tabla[i]!=null){
							if(tabla[i].name=="PRICE-SHORT-ROUTE"){
								flag=true;
								...
								if(tabla[i].tag=="PRICE-SHORT-ROUTE"){
									objeto=null;
	...
	app.PriceShortRouteReportComponent.prototype.formattedActions=function(data){
	...
	app.PriceShortRouteReportComponent.prototype.formattedView=function(data){
		if(data==null || data==undefined || data==""){
			return null;
		}else{
			if(data.name=="PRICE-SHORT-ROUTE-REPORT"){
				return null;
			}
			if(data.name=="PRICE-SHORT-ROUTE-CREATE"){
	...
	app.PriceShortRouteReportComponent.prototype.search=function(){
		this.jsonFilter={};
		var datos=[];
		var entity=null;
	...
	app.PriceShortRouteReportComponent.prototype.clean=function(){
		this.nombre=null;
	}
	app.PriceShortRouteReportComponent.prototype.getCantidadSelected=function(data){
		if (!(data == null || data == undefined || data == "")) {
	...
	app.PriceShortRouteReportComponent.prototype.getValueFirst=function(data){
		this.listRegister = [];
	...
	app.PriceShortRouteReportComponent.prototype.getValuePrevious=function(data){
		this.listRegister = [];
	...
	app.PriceShortRouteReportComponent.prototype.getValueLast=function(data){
		this.listRegister = [];
	...
	app.PriceShortRouteReportComponent.prototype.getValueNext=function(data){
		this.listRegister = [];
	...
	app.PriceShortRouteReportComponent.prototype.getValueChangeRecords=function(data){
		this.pageSelected = data;
	}
	app.PriceShortRouteReportComponent.prototype.callServices = function (data, parametros) {
		...
		let request = this.service.callServicesHttp("price-short-route-report", querys, this.jsonFilter);
	...
	app.PriceShortRouteReportComponent.prototype.procesarRespuesta=function(data){
		var key="results";
	...
	app.PriceShortRouteReportComponent.prototype.checkAction=function(data){
	...
	app.PriceShortRouteReportComponent.prototype.formattedData=function(data){
		if(data==null || data==undefined || data==""){
			return null;
		}else{
			...
			if(data.hasOwnProperty("price")){
				if(!(data.price==null || data.price==undefined || data.price=="")){
					data.monto=amountFormattingObject(data.price*100);
					data.monto=data.monto.integerPart+","+data.monto.decimalPart;
				}
			}
			return data;
		}
	}
	app.PriceShortRouteReportComponent.prototype.deselectedData = function() {
		this.dataSelected = null;
	}
	app.PriceShortRouteReportComponent.prototype.back=function(){
		window.history.back();
	}
