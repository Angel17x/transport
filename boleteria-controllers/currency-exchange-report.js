(function(app) {
	app.CurrencyExchangeReportComponent =
		ng.core.Component({
		selector: 'currency-exchange-report',
		templateUrl: 'views/currency-exchange-report.html',
		})
		.Class({
	...
	app.CurrencyExchangeReportComponent.prototype.ngOnInit=function(){
	...
	app.CurrencyExchangeReportComponent.prototype.checkRol=function(){
		this.actions=[];
		this.vistas=[];
		var flag=false;
			...
			if(this.service.getRole().hasOwnProperty("views")){
				if(!(this.service.getRole().views==null || this.service.getRole().views==undefined || this.service.getRole().views=="")){
					...
					for(var i=0;i<tabla.length;i++){
						if(tabla[i]!=null){
							if(tabla[i].name=="CURRENCY-EXCHANGE"){
								flag=true;
								...
								if(tabla[i].tag=="CURRENCY-EXCHANGE"){
									objeto=null;
	...
	app.CurrencyExchangeReportComponent.prototype.formattedActions=function(data){
	...
	app.CurrencyExchangeReportComponent.prototype.formattedView=function(data){
		if(data==null || data==undefined || data==""){
			return null;
		}else{
			if(data.name=="CURRENCY-EXCHANGE-REPORT"){
				return null;
			}
			if(data.name=="CURRENCY-EXCHANGE-CREATE"){
	...
	app.CurrencyExchangeReportComponent.prototype.search=function(){
		this.jsonFilter={};
		var datos=[];
		var entity=null;
	...
	app.CurrencyExchangeReportComponent.prototype.clean=function(){
		this.nombre=null;
	}
	app.CurrencyExchangeReportComponent.prototype.getCantidadSelected=function(data){
		if (!(data == null || data == undefined || data == "")) {
	...
	app.CurrencyExchangeReportComponent.prototype.getValueFirst=function(data){
		this.listRegister = [];
	...
	app.CurrencyExchangeReportComponent.prototype.getValuePrevious=function(data){
		this.listRegister = [];
	...
	app.CurrencyExchangeReportComponent.prototype.getValueLast=function(data){
		this.listRegister = [];
	...
	app.CurrencyExchangeReportComponent.prototype.getValueNext=function(data){
		this.listRegister = [];
	...
	app.CurrencyExchangeReportComponent.prototype.getValueChangeRecords=function(data){
		this.pageSelected = data;
	}
	app.CurrencyExchangeReportComponent.prototype.callServices = function (data, parametros) {
		...
		let request = this.service.callServicesHttp("currency-exchange-report", querys, this.jsonFilter);
	...
	app.CurrencyExchangeReportComponent.prototype.procesarRespuesta=function(data){
		var key="results";
	...
	app.CurrencyExchangeReportComponent.prototype.checkAction=function(data){
	...
	app.CurrencyExchangeReportComponent.prototype.formattedData=function(data){
		if(data==null || data==undefined || data==""){
			return null;
		}else{
			if(data.hasOwnProperty("created_at")){
				if(!(data.created_at==null || data.created_at==undefined || data.created_at=="")){
					data.formatted_at=formattingDate(data.created_at);
				}
			}
			if(data.hasOwnProperty("rate")){
				if(!(data.rate==null || data.rate==undefined || data.rate=="")){
					data.monto=amountFormattingObject(data.rate*100);
					data.monto=data.monto.integerPart+","+data.monto.decimalPart+" Bs";
				}
			}
			return data;
		}
	}
	app.CurrencyExchangeReportComponent.prototype.deselectedData = function() {
		this.dataSelected = null;
	}
	app.CurrencyExchangeReportComponent.prototype.selectedAction = function(data, action) {
	...
	app.CurrencyExchangeReportComponent.prototype.deleteData=function(){
	...
	app.CurrencyExchangeReportComponent.prototype.procesarRespuestaDelete=function(data){
		let mensajeAll=_("message_dflt_59");
		if (data == null || data == undefined || data == "") {
			this.mensaje = mensajeAll;
			this.msg.error();
	...
	app.CurrencyExchangeReportComponent.prototype.back=function(){
		window.history.back();
	}
